...
Cake = class('Cake',Entity)

function Cake:initialize(x,y)
	Entity:initialize()
	-- self.id = #ENTITIES + 1
	...
	self.scale = 2
	self.image = MapHandler:getAsset("Cake")
	self.image:setFilter("nearest")
	
	
	self.eat_sound = MapHandler:getAsset("CakeEat")
	self.eat_sound:setVolume(MapHandler:getVolume())
...
function Cake:CalculatePos(speed_mult)
